...
function FeatureCard({ icon: Icon, title, description }: { icon: React.ElementType; title: string; description: string }) {
  return (
    <div className="apple-card p-6 flex flex-col gap-3 group hover:border-primary/20 transition-all duration-300">
      ...
      <main className="min-h-[calc(100vh-3.5rem)]">
        ...
        <section className="flex flex-col items-center justify-center px-4 pt-24 pb-20 text-center relative">
          ...
          <div className="animate-fade-in relative z-10">
            ...
            <div className="flex gap-3 justify-center mt-8">
              ...
              <Link to="/settings">
                <Button size="lg" variant="outline" className="rounded-full px-7 h-12 text-[15px] font-medium border-border bg-card hover:bg-accent text-foreground">
                  Try Demo
                </Button>
              </Link>
            </div>
        ...
        <section className="container pb-24">
          <div className="grid gap-4 md:grid-cols-3">
            <FeatureCard icon={Blocks} title="Blockchain Verified" description="Every drug batch is cryptographically registered on-chain for immutable proof of authenticity." />
            <FeatureCard icon={Eye} title="Anomaly Detection" description="AI-powered scanning detects suspicious patterns like rapid scans and geographic anomalies." />
            <FeatureCard icon={Lock} title="Secure Supply Chain" description="End-to-end verification from manufacturer to pharmacy with complete audit trails." />
          </div>
        </section>
      </main>
    );
