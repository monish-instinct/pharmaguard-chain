  ...
  return (
    <main className="min-h-screen bg-background overflow-x-hidden">
      ...
      <div className="bg-gradient-to-b from-primary/[0.03] to-background border-b border-border">
        <div className="container max-w-lg py-10 px-4 text-center">
          <div className="mx-auto mb-4 flex h-14 w-14 items-center justify-center rounded-3xl bg-primary/[0.07]">
            <Shield className="h-7 w-7 text-primary" />
          </div>
          <h1 className="text-[24px] sm:text-[28px] font-bold tracking-tight text-foreground">
            Medicine Safety Check
          </h1>
          <p className="mt-2 text-[14px] text-muted-foreground max-w-xs mx-auto leading-relaxed">
      ...
      <div className="container max-w-lg py-6 px-4">
        <div className="flex flex-col gap-4">
          ...
          <div className="apple-card p-5">
            ...
            />
            {!scanning ? (
              <Button onClick={startScanner} className="w-full h-12 rounded-2xl text-[14px] font-semibold glow-primary gap-2.5">
                <Camera className="h-5 w-5" /> Scan Medicine QR Code
              </Button>
            ) : (
              <Button variant="outline" onClick={stopScanner} className="w-full h-11 rounded-xl text-[13px] font-medium border-destructive/20 text-destructive hover:bg-destructive/5">
                Stop Camera
              </Button>
            )}
          ...
          {verifyStatus && (
            <motion.div
              ...
              className="flex items-center gap-2.5 p-3.5 rounded-xl bg-primary/[0.03] border border-primary/10"
            >
              <Loader2 className="h-4 w-4 animate-spin text-primary" />
              <span className="text-[13px] text-foreground font-medium">{verifyStatus}</span>
            </motion.div>
          )}
          ...
          <AnimatePresence mode="wait">
            ...
            {isRecalled && (
              <motion.div
                ...
                className="apple-card border-2 border-destructive p-6 flex flex-col items-center gap-4 bg-destructive/[0.03]"
              >
                ...
                <h2 className="text-[18px] sm:text-[20px] font-bold text-destructive text-center">
                  🚨 THIS MEDICINE HAS BEEN RECALLED
                </h2>
                <p className="text-[13px] text-destructive/80 text-center font-medium">
                ...
                <Link to="/report">
                  <Button variant="destructive" className="rounded-xl gap-2 mt-1">
            ...
            {result && !isRecalled && (
              ...
              >
                {/* Large Safety Badge */}
                <div className={`apple-card border p-6 flex flex-col items-center gap-4 ${safetyConfig[result].bg} ${safetyConfig[result].glow}`}>
                  <div className={`flex h-20 w-20 items-center justify-center rounded-[24px] ${
                    ...
                    {React.createElement(safetyConfig[result].icon, { className: `h-10 w-10 ${safetyConfig[result].color}` })}
                  </div>
                  <div className="text-center">
                    <h2 className={`text-[20px] sm:text-[22px] font-bold ${safetyConfig[result].color}`}>
                  ...
                  {safetyScore !== null && (
                    <div className="w-full">
                      <div className="flex items-center justify-between mb-1.5">
                        <span className="text-[12px] font-medium text-foreground">Safety Score</span>
                        <span className={`text-[12px] font-bold ${safetyScoreLabel(safetyScore).color}`}>
                  ...
                  <div className="apple-card p-5">
                    ...
                    <div className="flex items-center justify-between gap-3">
                      <div className="min-w-0 flex-1">
                        <p className="text-[13px] font-medium text-foreground truncate">{trustScore.name}</p>
                        <p className="text-[11px] text-muted-foreground mt-0.5">Based on verification history</p>
                      </div>
                      <div className="text-right shrink-0">
                        <div className="flex items-center gap-0.5 justify-end">
                          {Array.from({ length: 3 }).map((_, i) => (
                            <Star
                              key={i}
                              className={`h-4 w-4 ${i < trustLevelFromScore(trustScore.score).stars ? 'text-warning fill-warning' : 'text-muted-foreground/20'}`}
                            />
                          ))}
                        </div>
                        <p className={`text-[12px] font-semibold mt-0.5 ${
                          ...
                          {trustLevelFromScore(trustScore.score).label}
                        </p>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="apple-card p-5">
                    ...
                    <div className="flex flex-col gap-2">
                      {risk.flags.map((flag, i) => (
                        <div key={i} className="text-[12px] text-foreground bg-warning/[0.04] border border-warning/15 rounded-xl px-3.5 py-2.5 flex items-start gap-2">
                          <AlertTriangle className="h-3.5 w-3.5 text-warning shrink-0 mt-0.5" />
                          <span className="break-words min-w-0">{flag}</span>
                        </div>
                      ))}
                    </div>
                  </div>
                ...
                {supplyEvents.length > 0 && (
                  <div className="apple-card p-5">
                    ...
                    {/* Simple visual journey steps */}
                    <div className="grid grid-cols-4 gap-1 mb-5">
                      {['Mfg', 'Dist', 'Pharmacy', 'You'].map((step, i) => {
                        ...
                        return (
                          <div key={step} className="flex flex-col items-center gap-1.5">
                            ...
                            <span className={`text-[10px] font-medium text-center ${reached ? 'text-foreground' : 'text-muted-foreground'}`}>
                              {step}
                            </span>
                          </div>
                        );
                    ...
                    {/* Connecting bar */}
                    <div className="flex items-center gap-0 mb-5 px-4">
                      {[0, 1, 2].map(i => {
                        const reached = i < Math.min(supplyEvents.length, 3);
                        return (
                          <div key={i} className={`h-1 flex-1 rounded-full ${reached ? 'bg-primary' : 'bg-muted'}`} />
                        );
                      })}
                    </div>
                ...
                {result === 'suspicious' && (
                  <Link to={`/report?batch=${scannedId}`}>
                    <Button variant="outline" className="w-full h-11 rounded-xl gap-2 border-warning/30 text-warning hover:bg-warning/5 text-[13px] font-medium">
                      <Flag className="h-4 w-4" /> Report This Medicine
                    </Button>
                  </Link>
                )}
  ...
  return (
    <div className="flex items-center gap-3 p-3 rounded-xl bg-muted/50">
      ...
      <div className="min-w-0 flex-1">
        ...
        <p className={`text-[13px] font-medium break-words ${highlight ? 'text-warning' : 'text-foreground'}`}>{value}</p>
      </div>
    </div>
  );
}
